...
def quality(y_test: List, 
    ...
    print('Score F1 : {}'.format(np.round(F1, 3)))
    print('AUC score : {}'.format(np.round(AUC, 3)))



def confusion_matrix(y_test: List,
                     y_prediction: List) -> pd.DataFrame :
...
def proba_seuil(liste_proba: List, seuil: float) -> List : 
...
def custom_metric(y_test: List, y_prediction: List) -> float : 
...
def score_seuil(prob_pos: float, y_validation: List) -> float:
    ...
    for i in np.arange(0.0001,1.1,0.025):
        ...
        pred = proba_seuil(prob_pos, i)

        my_score.append(custom_metric(y_validation, pred))
...
def custom_viz(y_test, y_pred_proba, model_name) -> None:
    ...
    plt.plot(list_sampled(false_positive_rate), list_sampled(true_positive_rate), label="{}".format(model_name))
    ...
    plt.title("Custom score", weight='bold', size=20)
